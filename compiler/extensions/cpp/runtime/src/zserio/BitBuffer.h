...
public:
     ...
     * Empty constructor.
     */
    BitBuffer(const ALLOC& allocator = ALLOC());
     ...
     */
    explicit BitBuffer(size_t bitSize, const ALLOC& allocator = ALLOC());
     ...
     */
    explicit BitBuffer(const std::vector<uint8_t, ALLOC>& buffer);
     ...
     */
    explicit BitBuffer(const std::vector<uint8_t, ALLOC>& buffer, size_t bitSize);
     ...
     * \param buffer STL vector of bytes from which the bit buffer should be created.
     */
    explicit BitBuffer(std::vector<uint8_t, ALLOC>&& buffer);
     ...
     * \throw CppRuntimeException If given bit size is out of range for given vector.
     */
    explicit BitBuffer(std::vector<uint8_t, ALLOC>&& buffer, size_t bitSize);
     ...
     * \param bitSize Number of bits stored in buffer to use.
     */
    explicit BitBuffer(const uint8_t* buffer, size_t bitSize, const ALLOC& allocator = ALLOC());
    ...
    BitBuffer(const BitBuffer<ALLOC>&) = default;
    BitBuffer& operator=(const BitBuffer<ALLOC>&) = default;

    BitBuffer(BitBuffer<ALLOC>&&) = default;
    BitBuffer& operator=(BitBuffer<ALLOC>&&) = default;
    /**
     * \}
     */
     ...
     */
    bool operator==(const BitBuffer<ALLOC>& other) const;
...
using BitBuffer = detail::BitBuffer<>;
